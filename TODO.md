...
- Sistema de rótulos nas transações: quero, na hora de criar uma despesa ou receita, poder selecionar vários badges/tags/rótulos para categorizar essas transações. O comportamento é justamente esse, um campo multiseletor onde posso vincular esses rotulos a uma transação. A exibição disso na tabela, eu quero que seja uma coluna de "Tags" ou "Rótulos", onde vai aparecer na célula da coluna a primeira tag/badge/rotulo, e se houver mais de 1 tag, eu quero que apareça um badge cinza com "...", e ele será hover, ou seja, quando eu passar o mouse em cima, deve aparecer um balão contendo todas as tags vinculadas naquela transação. Nesse balão. Esta célula deve ser editável como todas as outras, só que a diferença é que nela, eu quero um botão com sinal de "+", redondinho, que quando clico abre um menu de contexto/componente/balão pra eu selecionar as tags que eu quiser adicionar ou se eu reemover, quero remover por ali também, tipo um "toggle". Então poderei editar pela tabela ou pelo formulário de transação também. No caso de haver mais de 1 tag, o botão redondinho com "+" deve aparecer dentro do balão com as tags caso eu queria adicionar novas tags ou remover alguma. A ideia é usar esses rotulos num sistema de filtragem futuro que ainda não vamos trabalhar agora. A coluna de tags/rótulos deve permitir ordenação, ou seja, quando eu clicar pra ordenar por "Rótulos", o sistema deve permitir isso, por isso os rótulos devem ter uma espécie de precedencia (pode ser definida pelo timestamp de quando ele foi criado, não é relevante isso). Ao ordenar, os registros devem ficar agrupadinhos. Outra coisa, existe um rótulo automático, que no caso seria o rótulo de "Rateio", que hoje é exibido como "[Rateio]" concatenado na descrição, eu quero trocar isso pra que seja badge automático, os demais badges eu quero poder cadastra-los conforme a minha necessidade, só não sei exatamente onde posso ter essa gestão de tags/rótulos/badges, talvez isso pudesse ficar em um menu de "configuração" do Jolia, na barra lateral, acima do "Trocar perfil" e isso direcionar pra uma tela onde posso ter várias configurações futuramente, mas por hora, lá pode ficar a lista de badges cadastrados. Ah! Lembrando que quero poder escolher as cores dos badges, portanto isso deve constar na modal de cadastro dos badges. Quero poder edita-los (o nome, cor) e também quando excluir um badge, quero que o sistema veerifique se ele foi vinculado à alguma transação, e se foi, nao deve permitir a exclusão, obrigando o usuario a desvincular antes. Lembrando que, existe o histórico de transações passadas, que podem ter badges vinculados, nesse caso, o usuário nao conseguiria "desvincular" os badges de transações passadas, eu aceito sugestões do que fazer nesse cenário pois nao cheguei a uma conclusão do modelo ideal nesse cenário. Monte um plano de ação para implementar essa feature, quero que detalhe e documente ela para que eu aprove ou modifique detalhes da implementação antes de começarmos.
