...
import { MoreVertical, CheckCircle, XCircle, Edit, Trash2, ArrowUpDown, Repeat, Users, Info } from 'lucide-react';
        ...
        <div className="md:hidden border border-border-color rounded-lg mb-4 p-4 space-y-4 bg-card hover:bg-background/50">
            <div className="flex justify-between items-start">
                 <div className="font-medium text-text-primary flex items-center gap-2 pr-2 overflow-hidden">
                     ...
                     <EditableCell value={item.description} onSave={(v) => onUpdateField(item.id, 'description', String(v))} disabled={isClosed || isApportioned} />
                </div>
                 {!isClosed && !isApportioned && (
            ...
            <div className="flex justify-between items-center text-sm">
                <div className="text-text-secondary">
                 ...
                 <button onClick={() => onTogglePaid(item)} className="inline-flex items-center transition-opacity hover:opacity-80 disabled:cursor-not-allowed disabled:opacity-70" disabled={isClosed || isApportioned}>
...
export const TransactionTable: React.FC<TransactionTableProps> = (props) => {
    const { title, data, type, isClosed, requestSort, sortConfig, subprofileRevenueProportions, subprofiles, apportionmentMethod, ...rest } = props;
        ...
        <Card>
            ...
            <CardContent>
                <div className="md:hidden">
                    {data.length > 0 && data.map(item => <TransactionItem key={item.id} item={item} type={type} isClosed={isClosed} {...rest} onTogglePaid={() => rest.onTogglePaid(item)} />)}
                                ...
                                <tr key={item.id} className={`bg-card ${!isApportioned && 'hover:bg-background'}`}>
                                    <td className="px-4 py-3 align-middle font-medium text-text-primary">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <EditableCell value={item.description} onSave={(newValue) => props.onUpdateField(item.id, 'description', String(newValue))} disabled={isClosed || isApportioned} />
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-4 py-3 align-middle"><EditableCell value={item.planned} type="number" formatAsCurrency onSave={(newValue) => props.onUpdateField(item.id, 'planned', newValue as number)} disabled={isClosed || isApportioned}/></td>
                                    <td className="px-4 py-3 align-middle"><EditableCell value={item.actual} type="number" formatAsCurrency onSave={(newValue) => props.onUpdateField(item.id, 'actual', newValue as number)} disabled={isClosed || isApportioned}/></td>
                                    ...
                                    <td className="px-4 py-3 text-center align-middle">
                                        <button 
                                            ...
                                            disabled={isClosed || isApportioned && type !== 'income'}
                                        >
                                            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium flex items-center ${item.paid ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300'}`}>
                                                {item.paid ? <CheckCircle className="w-4 h-4 mr-1"/> : <XCircle className="w-4 h-4 mr-1"/>}
                                                {item.paid ? 'Sim' : 'Não'}
                                    ...
                                    <td className="px-4 py-3 text-center align-middle">
                                        {!isClosed && !isApportioned && <ActionMenu item={item} onEdit={props.onEdit} onDelete={props.onDelete} />}
                                    </td>
                                </tr>
                                )})}
                        </tbody>
